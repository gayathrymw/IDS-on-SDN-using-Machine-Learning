...
class IDS_Application(simple_switch_13.SimpleSwitch13):
	...
	def __init__(self, *args, **kwargs):
		...
		file = open("/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictFlowStatsfile.txt","a+")
		...
		file = open("/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictPortStatsfile.txt","a+")
	...
	def data_cleaning_flow(self, input_path,output_path):
		...
		for line in file:
			...
			if first_line_flag_flow:
				...
				rows = [a[i : i + 5] for i in range(0, len(a), 5)]
				for row in rows:
					...
					if key in dict:
						...
						for i in range(len(ab)):
							if i == 2:
								...
								c[i] = int(row[i]) - int(b[i])
						file1.write(str(c[3]) + "," + str(c[4]) + "\n")
						#dict[key] =( str(a[0])+ "," +str(a[1]) + "," + str(a[2])+ "," + str(a[3])+"," + str(a[4]))
						dict[key] = f"{row[0]},{row[1]},{row[2]},{row[3]},{row[4]}"
					else:
						#dict[key] = (str(a[0])+ "," +str(a[1]) + "," + str(a[2])+ "," + str(a[3])+"," + str(a[4]))
						dict[key] = f"{row[0]},{row[1]},{row[2]},{row[3]},{row[4]}"
						file1.write(str(row[3]) + "," + str(row[4]) + "\n")
		file.close()
		file1.close()
	...
	def data_cleaning_port(self, input_path, output_path):
		...
		for line in file:
			a = line.split(",")
			if len(a)>2:
				if "rx_bytes" in line and "rx_pkts" in line and "tx_bytes" in line and "tx_pkts" in line:
					continue
	...
	def IDS_training(self):
		...
		self.data_cleaning_flow('/home/gayathry/IDS-on-SDN-using-Machine-Learning/FlowStatsfile.txt','/home/gayathry/IDS-on-SDN-using-Machine-Learning/FlowStatssfile_cleaned.txt')
		flow_without_key = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/FlowStatsfile_cleaned.txt')
		flow_stat_input_target = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/FlowStatsfile_target.txt')

		# Read cleaned port statistics data
		self.data_cleaning_port('/home/gayathry/IDS-on-SDN-using-Machine-Learning/PortStatsfile.txt','/home/gayathry/IDS-on-SDN-using-Machine-Learning/PortStatssfile_cleaned.txt')
		port_without_key = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/PortStatsfile_cleaned.txt')
		port_stat_input_target = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/PortStatsfile_target.txt')
		...
		print("not here______________________________")
		...
		print("fine till here\n\n")
		print("Number of samples in port_without_key:", len(port_without_key))
		print("Number of samples in port_stat_input_target:", len(port_stat_input_target))

		rf_port.fit(port_without_key[:2616],port_stat_input_target[:2616].values.ravel())
		print("eeda okay ano??????????????????")
	...
	def IDS_impl(self):
		self.data_cleaning_flow('/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictFlowStatsfile.txt','/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictFlowStatsfile_cleaned.txt')
		self.data_cleaning_port('/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictPortStatsfile.txt','/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictPortStatsfile_cleaned.txt')	
		
		#Read cleaned file
		flow_predict_without_key = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictFlowStatsfile_cleaned.txt')
		port_predict_without_key = pd.read_csv('/home/gayathry/IDS-on-SDN-using-Machine-Learning/MalPredictPortStatsfile_cleaned.txt')
